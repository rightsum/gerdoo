...
| ~~**B6**~~ | ~~**COB LED strip voltage**~~ | ✅ **DONE 2026-08-26** | **It is a 5V strip.** Now on a dedicated MINI560 rail, switched by a D4184 MOSFET from Teensy pin 4. See [log 012](logs/012-2026-08-26-led-strip-ambient-brightness.md) | — |
| **B9** | **COB LED strip current at full** | next bench session | Meter **in series with the strip** at full brightness. Sizes the fuse and confirms MINI560 headroom | Currently only inferred from a ~200 mV pack sag — indirect |
| **B10** | **Is Teensy `VIN` fed from the LM2596S rail?** | before any 6V change | Trace the wire. Must be **unwired** | ⚠️ **Teensy 4.1 VIN max is 5.5V.** Raising the servo rail to 6V with VIN connected destroys the board |
| **B11** | **LocateAnything real throughput** | when convenient | Time one grounding query on the Orin with `llama-mtmd-cli`. Model and a CUDA build (sm_87) are already installed and have **never been run** | Closes G6 check 3 — the ~1–2 s/query figure there is scaled from an H100, not measured |
| **B12** | **Wake-word range with a better mic** | if 4 m is not enough | The Brio caps at ~4 m; software gain cannot improve signal-to-noise. Try a USB conference mic | See [log 013](logs/013-2026-08-29-audio-and-persian-wake-word.md) |
| **B13** | **USB speakerphone with hardware AEC** | if barge-in degrades | One device = one clock domain, which is what echo cancellation actually needs. Would also solve B12 | Software AEC across two independent USB clocks is inherently marginal — see [log 014](logs/014-2026-09-01-livekit-voice-agent.md) |
